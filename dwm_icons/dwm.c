...
drawstatusbar(Monitor *m, int bh, char* stext) {
    ...
    /* Subtract getsystraywidth() to shift the entire status bar to the left */
...
loadxrdb()
{
  ...
  char *type;
  XrmValue value;
  system("xrdb ~/.config/X11/Xresources");
  ...
  if (display != NULL) {
    ...
    if (resm != NULL) {
      ...
      if (xrdb != NULL) {
        ...
        XRDB_LOAD_COLOR("dwm.tagsselcolor",  tagsselcolor);
        XRDB_LOAD_COLOR("dwm.tagsnormcolor", tagsnormcolor);
        XRDB_LOAD_COLOR("dwm.infofgcolor",   infofgcolor);
        XRDB_LOAD_COLOR("dwm.infonormcolor", infonormcolor);
      }
    }
  }
...
updatesystrayicongeom(Client *i, int w, int h)
{
	if (i) {
		...
		if (w == h)
			...
			i->w = (int) ((float)(bh - 15) * ((float)w / (float)h));
		...
		if (i->h > bh - 15) {
			if (i->w == i->h)
				...
				i->w = (int) ((float)(bh - 15) * ((float)i->w / (float)i->h));
			i->h = bh - 15;
		}
	}
}
...
updatesystray(void)
{
	...
	for (w = 0, i = systray->icons; i; i = i->next) {
		...
		w += systrayspacing;
		i->x = w;
		XMoveResizeWindow(dpy, i->win, i->x, 8, i->w, i->h);
		w += i->w;
		if (i->mon != m)
			i->mon = m;
	}
