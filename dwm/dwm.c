...
drawstatusbar(Monitor *m, int bh, char* stext) {
    ...
    /* subtract getsystraywidth to avoid overlap */
...
updatesystrayicongeom(Client *i, int w, int h)
{
	if (i) {
		...
		if (w == h)
			...
			i->w = (int) ((float)(bh - 18) * ((float)w / (float)h));
		...
		if (i->h > bh - 18) {
			if (i->w == i->h)
				...
				i->w = (int) ((float)(bh - 18) * ((float)i->w / (float)i->h));
			i->h = bh - 18;
		}
	}
}
...
updatesystray(void)
{
	...
	for (w = 0, i = systray->icons; i; i = i->next) {
		...
		w += systrayspacing;
		i->x = w;
		XMoveResizeWindow(dpy, i->win, i->x, 10, i->w, i->h);
		w += i->w;
		if (i->mon != m)
			i->mon = m;
	}
